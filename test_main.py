...
def test_api_key():
    assert len(api_key) == 32, "API-key should be 32 characters long."

def test_steam_id():
    assert len(steam_id) == 17, "Steam-ID should be 17 digits long."
    
def test_status():
    assert response.status_code == 200, f"Request has failed with status code: {response.status_code}"
